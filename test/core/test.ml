...
let run_test : test -> outcome Lwt.t = fun test ->
  if test.skip_if_this_is_false () = false then
    ...
    Lwt.register_pause_notifier (fun _paused_count ->
      Lwt.bind Lwt.return_unit (fun () ->
      Lwt.wakeup_paused ();
      Lwt.return_unit)
      |> ignore);
...
external on_unhandled_rejection :
  (_ [@bs.as "unhandledRejection"]) -> (unit -> unit) -> unit =
  "process.on"
    [@@bs.val]

let () =
  on_unhandled_rejection ignore
